...
public class ISummonNoobsBackendServiceTests
{
    ...
    public async Task MessageDequeued_NotifierCalled()
    {
        ...
        var stateManager = new MockReliableStateManager();
        var service =
            new ISummonNoobsBackendService.ISummonNoobsBackendService(context, stateManager, clusterNotifierMock.Object)
            {
                WaitTimeBetweenLoop = TimeSpan.FromMilliseconds(10)
            };
    ...
    public async Task PrimaryReplicaTest()
    {
        ...
        var replicaSet = new MockStatefulServiceReplicaSet<ISummonNoobsBackendService.ISummonNoobsBackendService>(
            (sc, stateManagerReplica2) => new ISummonNoobsBackendService.ISummonNoobsBackendService(sc, stateManagerReplica2,
