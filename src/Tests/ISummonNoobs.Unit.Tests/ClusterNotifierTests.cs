...
namespace ISummonNoobs.Unit.Tests
{
    ...
    public class ClusterNotifierTests
    {
        ...
        {
            var item= (StatelessService)Activator.CreateInstance(typeof(StatelessService), BindingFlags.Instance | BindingFlags.NonPublic, (Binder)null, new object[6]
            {
                (object) serviceName,
                (object) serviceTypeName,
                (object) serviceManifestVersion,
                (object) healthState,
                (object) serviceStatus,
                (object) isServiceGroup
