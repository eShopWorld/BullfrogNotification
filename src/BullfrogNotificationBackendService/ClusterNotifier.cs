...
{
    /// <summary>
    /// this is the core component of the distribution
    ///
    /// the logic
    ///  - iterate over all apps, services (APIs are stateless), partitions (API=singular partition) and replicas (primary only)
    ///  - for healthy partition (warning is accepted) - when HTTP endpoint is found, attempt call to message ingest
    /// some assumptions
    ...
    public class ClusterNotifier
    {
        ...
        private const string MessageIngestUrlFormat = "notification/{0}";
